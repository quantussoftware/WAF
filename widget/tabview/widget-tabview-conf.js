...
WAF.addWidget({    
    ...
    },
    {
        ...
        onchange    :function(data){
            ...
            tag.setMenuPosition(this.getValue());
        }
    },
    {
        name        : 'data-tabs',
        ...
        afterRowAdd : function(data) {
            ...
            data.items[0].setValue('[Tab ' + newItem.getIndex() + ']');
        },
        afterRowDelete : function(data) {
            ...
            menuItem    = data.items[0].getData().tab;
    ...
    onCreate : function (tag, param) {        
        ...
        tag._containerResize = function (e, paramSize) {
            ...
            if (tabView.status != 'destroy') {
                ...
                /*for (i = 0; i < linksL; i += 1) {
                    ...
                    if (link != this && link.getType() == 'container') {
                        if (paramSize.width) {
                            link.setWidth(paramSize.width);
                        }

                        if (paramSize.height) {                            
                            link.setHeight(paramSize.height);

                            tabView.setHeight(tabView.getHeight() - (paramSize.height-paramSize.oldHeight));
                        }
                    }
                }*/

                        if (paramSize.width) {    
                            //tabView.setWidth(tabView.getWidth() - (paramSize.width-paramSize.oldWidth), false, false);
                        }

                        if (paramSize.height) {    
                            //console.log(tabView.getHeight() , paramSize.height, paramSize.oldHeight)
                            //tabView.setHeight(tabView.getHeight() - (paramSize.height-paramSize.oldHeight), false, false);

                            //console.log(tabView.getHeight())
                        }
        ...
        if (!param._isLoaded) {   
            ...
            jQMenu.bind('onResize', tag._menuResizeFn);
            ...
            jQTag.bind('onReady', function(){
                ...
                newGroup    = false;
                /*
                 * Create group if doesn't exists
                 */
                if (groupId == -1) {
                    group = new Designer.ui.group.Group();
                    group.add(that);
    ...
    afterAddTab : function(e, menuItem, silentMode) { 
        ...
        container.savePosition('left', containersInfo.left, false, false);

        container.savePosition('right', containersInfo.right, false, false);

        container.savePosition('top', containersInfo.top, false, false);

        container.savePosition('bottom', containersInfo.bottom, false, false);

        jQContainer
            .bind('onWidgetDestroy', tag._containerAfterDestroy)
            .bind('onResize', tag._containerResize);
