	...
	if (event.eventKind == 'onCollectionChange') {
		...
		if (event.eventKind == "onSelectionChange") {
			...
			if (event.eventKind == "onAttributeChange") {
				...
				if (event.eventKind == 'onBeforeCurrentElementChange') {
					var posInSource = event.dataSource.getPosition();
					gridView.drawRow(posInSource, event.dataSource, true);
					if (gvars.needAutoSave) {
						gvars.needAutoSave = false;
						event.dataSource.save({onError: gridView.gridController.errorHandler || null});
					}
				}
				else 
					if (event.eventKind == 'onCurrentElementChange') {
						...
						if (event.eventKind == 'onElementSaved') {
		...
		functions: {
			...
			initWithOptions: function(options){
				...
				if (options.hideFooter == 'true') {
					...
					gridView._private.globals.footerHeight = 30;
				}
			...
			updateViewportSize: function(options){
				...
				var gridView = options.gridView, viewportTop, viewportBottom;
			...
			updateHeaderSize: function(options){
				...
				(WAF.PLATFORM.modulesString === "mobile") ? gridView._private.globals.headerNode.height(25) : gridView._private.globals.headerNode.height(32);
			},
				...
				gridView.gridController.dataGrid.dataSource.select(row.rowNumber, {
					onSuccess: function(event){
						if (!!gridView.gridController.onRowRightClick) {
							rightClickReturn = gridView.gridController.onRowRightClick(row.rowNumber, righClickEvent);
						}
					}
				});
								...
								if (sourceAtt != null) {
									if (WAF.PLATFORM.isTouch && WAF.PLATFORM.OS === "iOs" && WAF.PLATFORM.OSVersion >= 5) {
										if (gridView._private.globals.promptedValue != null) {
											...
											newvalue = sourceAtt.normalize(editInfo.editArea[0].value.replace(/-/gi, "/"));
										}
									}
									else {
...
WAF.classes.GridController = function(){
	...
	this.baseDblClick = function(event) {
		...
		if (dblclickHandler != null) {
			var row = event.data.row;
			if (row.rowNumber < gridView._private.globals.totalRowCount) {
				gridView.gridController.onRowDblClick(row.rowNumber);
			}
		}
	...
	this.onRowRightClick = function(rowNumber, rightClickEvent){
...
WAF.Widget.provide('Grid', {
	...
	errorDiv = config['data-errorDiv'], hideFooter = false, 
